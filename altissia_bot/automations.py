...
def login(page: Page, username: str, password: str) -> bool:
    ...
    try:
        ...
        page.click('button[type="submit"], button:has-text("Connexion"), button:has-text("Login"), button:has-text("Se connecter")')
...
def get_available_exercises(page: Page) -> list:
    ...
    try:
        ...
        exercise_elements = page.locator('[data-exercise], .exercise-item, a[href*="exercise"]').all()
    ...
    try:
        ...
        try:
            ...
            if len(correct_answers) == 1:
                ...
                print_success(f"Réponse {question_number} : {correct_answers} ({len(correct_answers)} trous)")
...
def fill_answer(page: Page, answers, question_number: int) -> bool:
    ...
    try:
        ...
        if len(input_fields) != len(answers):
            print_error(f"Nombre de champs ({len(input_fields)}) ≠ nombre de réponses ({len(answers)})")
            return False
